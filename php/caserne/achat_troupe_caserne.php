...
if($_POST)
{
	...
	{ 		
		...
		$race = htmlentities($prix_achat['race']);

		
		$soldat_preforme = $a_p_d['soldat'];
		$goldstock = $ressource_disponible['gold'];
		$titanestock = $ressource_disponible['titane'];
		$oriniastock = $ressource_disponible['orinia'];
		...
		if ($nombre_demande >= 0) {													// Permet de vérifier qu'il n'entre pas un nombre négatif
			...
			if ($soldat_preforme >= $nombre_demande) {																// Permet de vérifier qu'il possède assez de soldat		
						...
						{	
							...
							if($unite_possede > 0)
								{	
											...
											$stock_soldat_preforme = $soldat_preforme-$nombre_demande;					// CALCUL DISPONIBILITE SOLDATS
											$req_deduction_soldat_preforme=$bdd->prepare("UPDATE population SET soldat = $stock_soldat_preforme WHERE id_planete = ?");
					...
					$_SESSION['error'] = '<p class="red">Vous n\'avez pas assez de soldats préformé.</p>';
		}
		else
...
header('Location: '.pathView().'caserne/caserne_' . $race . '.php');
