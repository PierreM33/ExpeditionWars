...
$id_membre=htmlentities($_SESSION['id']); 
			...
			//$_POST['objet_deux'] == Nom de l'objet récupéré dans le champs
			...
			$q->execute(array(strip_tags($_POST['objet_deux'])));
			$o=$q->fetch();
			
			//ajout des statistiques au vaisseau
			$update=$bdd->prepare('UPDATE vaisseau_joueur SET attaque = attaque+? , defense=defense+? , bouclier = bouclier+?, objet_deux=? , nombre_objet_deux=? WHERE id = ?');
			$update->execute(array(strip_tags($o['attaque']),strip_tags($o['defense']),strip_tags($o['bouclier']),strip_tags($_POST['objet_deux']),1,strip_tags($_POST['numero'])));
			...
			$_SESSION['error'] = '<p class="green">L\'objet ' . strip_tags($_POST['objet_deux']) . ' vient d\'être ajouté au vaisseau.</p>';
	}
	else
