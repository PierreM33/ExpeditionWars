	 ...
	 if(!empty($_POST['id_cache'])) 
		...
		$Fa_L=$faction_loi->fetch();
			...
			else
			{
				...
				$limite=htmlentities($af_v['limite_chasseur']);
							...
							if($vaisseau_possede > 0)
								{
									...
									$D=$bdd->prepare('INSERT INTO construction_vaisseau (id_membre,id_planete,nom_unite,nombre_formation,temps,id_hangar,attaque,defense,bouclier,vitesse,type,fret,poid,gabarit,chasseur,limite_chasseur) VALUES(?,?,?,?,?,? ,?,?,?,?,? ,?,?,?,?,?)');
									$D->execute(array($id_membre,$planete_utilise,$nom_vaisseau,$nombre_demande,$temps,$id_spatial,$attaque,$defense,$bouclier,$vitesse,$type_vaisseau,$fret,$poid,$gabarit,$chasseur,$limite));
