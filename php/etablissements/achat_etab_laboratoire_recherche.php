...
$post=strip_tags($_POST['id_cache']);
		...
		$req_ressource_disponible=$bdd->prepare('SELECT * FROM ressource WHERE id_planete = ? AND id_membre = ?'); 
		$req_ressource_disponible->execute(array($planete_utilise,$id_membre));
		$ressource_disponible=$req_ressource_disponible->fetch();
		
		$req=$bdd->prepare('SELECT * FROM etablissement_joueur WHERE id_planete = ? AND joueur = ? AND id_etab = ?'); 
...
//On verifie si un batiment est déj en construction

	if( $construction == 0)
		{
			if (($goldstock >= $prix_gold) AND ($titanestock >= $prix_titane) AND ($cristalstock >= $prix_cristal) AND ($oriniastock >= $prix_orinia))
				{	
						...
						$req_up_construc=$bdd->prepare('UPDATE etab_evolution SET evolution = ? WHERE id_membre = ? AND id_planete = ?');
						$req_up_construc->execute(array(1,$id_membre,$planete_utilise));
