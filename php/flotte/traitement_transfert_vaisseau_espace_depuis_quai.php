								...
								if($Coord_planete_habite['espace'] == 1) // Si les coordonnée sont celle d'une case espace.
									{
										if(!empty($nouvelle_position))
											{				
													...
													$upvais=$bdd->prepare('UPDATE vaisseau_joueur SET id_action = ?, case_espace = ?, case_planete = ?	WHERE id_membre = ? AND id_planete = ? AND id = ?');
													$upvais->execute(array($action_id,1,0, $id_membre, $planete_utilise, $id_vaisseau));
													
													//N VA UPDATE SES CHASSEURS EGALEMENT
													$upcha=$bdd->prepare('UPDATE vaisseau_joueur SET id_action = ?, case_espace = ?, case_planete = ?	WHERE id_membre = ? AND id_planete = ? AND id_vaisseau_hote = ?');
													$upcha->execute(array($action_id,1,0, $id_membre, $planete_utilise, $id_vaisseau));
...
header('Location: '.pathView().'./flotte/baser_vaisseau_quai.php');
?>